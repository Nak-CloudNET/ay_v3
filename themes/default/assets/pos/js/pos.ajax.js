...
function loadItems() {
	...
	if (localStorage.getItem('positems')) {
		...
		total_discount = 0;
		var dis_item = 0;
		var gp = jQuery.parseJSON(localStorage.getItem('gp'));
		var admin = localStorage.getItem('admin');
		var owner = localStorage.getItem('owner');
		if(gp){
			dis_item = gp["sales-discount-item"];
		}
		...
		$.each(positems, function (i, e) {
			...
			percent_cus_group = (item.row.percent_cus_group == 0 || item.row.percent_cus_group == null || item.row.percent_cus_group == undefined) ? 0 : item.row.percent_cus_group.substring(1)+'%';
			...
			if (ds.indexOf("%") !== -1) {
				...
				if (!isNaN(pds[0])) {
					item_discount = (parseFloat(unit_price) * parseFloat(pds[0])) / 100;
					discount_item = formatMoney(((parseFloat(unit_price) * item_qty) * parseFloat(pds[0])) / 100)
				} else {
					item_discount = formatDecimal(ds);
					discount_item = formatDecimal(ds);
				}
			} else {
				item_discount = parseFloat(ds);
				discount_item = parseFloat(ds);
			}
			...
			/*if(percent_cus_group != 0){
				tr_html += '<td class="text-center"><span>'+ formatMoney(before_unit_price) +'</span></td>';
			}*/
			...
			if (item_promotion == 1){
				tr_html += '<input class="rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + item_pro_price + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + item_pro_price + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + item_pro_price + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + formatMoney(parseFloat(item_pro_price)) + '</span></td>';
			}else{
            ...
            if (item_promotion == 1){
				...
				tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney(((parseFloat(unit_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty)) - discount_item) + '</span><input type="hidden" name="grand_total[]" value="' + formatMoney(((parseFloat(item_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty))) + '"></td>';
			}
			...
			if (item_promotion == 1){
				...
				total += formatDecimal(((parseFloat(unit_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty)) - discount_item);
			}
	...
	if(susp_id > 0){
		...
		if (posdiscount = localStorage.getItem('posdiscount') ? localStorage.getItem('posdiscount') : percent_cus_group) {
			$('#percent_cus_group').val(percent_cus_group);
			var ds = posdiscount ? posdiscount : percent_cus_group;
			if (ds.indexOf("%") !== -1) {
				var pds = ds.split("%");
				if (!isNaN(pds[0])) {
		...
		total_discount = formatMoney(order_discount + product_discount);
