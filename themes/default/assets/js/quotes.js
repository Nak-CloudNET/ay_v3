...
function loadItems() {
    ...
    if (localStorage.getItem('quitems')) {
        ...
        total_discount = 0;
		percent_cus_group = 0;
		get_order_dis = localStorage.getItem('qudiscount');
        $("#quTable tbody").empty();
        ...
        if (qudiscount = localStorage.getItem('qudiscount') ? localStorage.getItem('qudiscount') : percent_cus_group) {
			if(percent_cus_group){
			   $('#qudiscount').val(percent_cus_group);
			}
            var ds = qudiscount ? qudiscount : percent_cus_group;
			console.log(ds);
            if (ds.indexOf("%") !== -1) {
                var pds = ds.split("%");
                if (!isNaN(pds[0])) {
