...
$(document).on('keyup', '#amount_1', function (e) {
	if ((e.keyCode >= 48 && e.keyCode <= 57) || (e.keyCode >= 96 && e.keyCode <= 105)) {
		...
		alert("Paid amount is not type of number");
		$(this).val(0);
	}
	...
	if (localStorage.getItem('slitems')) {
		...
		$.each(slitems, function () {
			...
			if (item_promotion == 1){
				...
				tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney(((parseFloat(unit_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty)) - (item_discount)) + '</span><input type="hidden" name="grand_total[]" value="'+ formatMoney(((parseFloat(unit_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty))  - (item_discount)) +'"></td>';
			}
			...
			if (item_promotion == 1){
				total += formatDecimal(((parseFloat(unit_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty)) - (item_discount));
			}else{
				total += formatDecimal(((parseFloat(unit_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty)) - (item_discount), 4);
			}
