...
<head>
	...
	<script>
		...
		localStorage.setItem('gp', '<?= json_encode($GP) ?>');
        ...
        <div class="container bblack" id="container">
            ...
            <div class="header-nav-pos">
                ...
                <ul class="nav navbar-nav pull-right">
					...
					<?php if ($Owner || $Admin || $GP['close_register']) { ?>
						<li class="dropdown">							
							<a href="#" id="close_register" title="<?= lang('close_register') ?>" class="btn borange pos-tip">
					...
					<?php } ?>
                        <div id="print">
                            <div id="left-middle">
								...
								<?php } ?>
                                        ...
                                        <tr>
											...
											<?php } ?>
											<!-- <th width="10%" class="before_unit_price"><?= lang("unit_price"); ?></th> -->
                                            <th width="10%"><?= lang("price"); ?></th>
											<th width="12%"><?= lang("price_kh"); ?></th>
                                            <th width="10%"><?= lang("qty"); ?></th>
											<?php if ($Admin || $Owner || $GP['sales-discount-item']) { ?> 
                            ...
                            <div id="left-bottom">
								...
								<input name="percent_cus_group" type="hidden" id="percent_cus_group">
                ...
                <table class="table table-bordered table-striped table-condensed table-hover"
					...
					<?php if ($Owner || $Admin || $GP['close_register']) { ?>
						<tr>
							...
							<td>
								<button type="button" class="btn bdarkGreen col-md-12" data-placement="bottom" data-html="true" href="<?= site_url('pos/close_register') ?>" data-toggle="modal" data-target="#myModal"> <?= $pos_settings->close_register ?></button>
							</td>
						</tr>
					<?php } ?>
					<tr>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
		$('#close_register').click(function(event){
			...
			bootbox.prompt({
				...
				callback: function (result) {
					$.ajax({
						type: 'get',
						url: '<?= site_url('auth/checkComfrimPWD'); ?>',
						dataType: "json",
						data: {
							password: result,
							id : user_id
						...
						success: function (data) {
							if(data){
								$('#myModal').modal({remote: site.base_url + 'pos/close_register/0/'+data});
								$('#myModal').modal('show');
							}else{
		...
		$('#saleman').live('change keyup paste', function() {
			$('#saleman_1').val($(this).val());
		}).trigger('change');
        ...
        function autoCalcurrencies(total_p){
            $(".curr_tpay").each(function(){
                ...
                $(this).html(formatMoney(total_p*rate));
            });
        }
		...
		/*$(document).on('change', '.poscustomer', function () {
			...
			$.ajax({
				type: "get",
				async: false,
				url: "<?= site_url('customers/suggestionsCustomerGroup') ?>/" + custom_id,
				dataType: "json",
				success: function (data) {
					if (data.results == null || data.results == 0) {
						$(".before_unit_price").remove();
					}
				}
			});
        });		*/
