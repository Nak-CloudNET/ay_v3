            ...
            $.each(reitems, function () {
                ...
                if (ds.indexOf("%") !== -1) {
                    ...
                    if (!isNaN(pds[0])) {
						...
						item_discount = item_discount * item_qty;
					} else {                        
                ...
                if (site.settings.product_discount == 1) {
                    tr_html += '<td class="text-right"><input class="form-control input-sm rdiscount" name="product_discount[]" type="hidden" id="discount_' + row_no + '" value="' + item_ds + '"><span class="text-right sdiscount text-danger" id="sdiscount_' + row_no + '">' + formatMoney(0 - (item_discount)) + '</span></td>';
                }
                if (site.settings.tax1 == 1) {
				...
				tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney(((parseFloat(item_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty)) - (item_discount) ) + '</span></td>';
                ...
                total += parseFloat((item_price + parseFloat(pr_tax_val)) * parseFloat(item_qty) - (item_discount));
	...
	function billerChange(){
        ...
        $.ajax({
            ...
            success: function(result){
				...
				$('#slwarehouse').val('<?= $warehouses->id; ?>').trigger('change');
                //$("#slwarehouse").select2("val", "<?=$Settings->default_warehouse;?>");
            }
        });
    }
									...
									<?php if ($Owner || $Admin || !$this->session->userdata('warehouse_id')) { ?>
										<div class="col-md-4">
											<div class="form-group">
												...
												echo form_dropdown('warehouse', $wh, (isset($_POST['warehouse']) ? $_POST['warehouse'] : ''), 'id="slwarehouse" class="form-control input-tip select" data-placeholder="' . lang("select") . ' ' . lang("warehouse") . '" required="required" style="width:100%; pointer-events: none;" ');
												?>
											</div>
										</div>
									<?php } else {
