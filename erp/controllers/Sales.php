...
class Sales extends MY_Controller
{
    ...
    function add($sale_order_id = NULL, $delivery_id = NULL, $quote_ID = NULL)
    {
		...
		if($quote_ID){
			...
			$sale_q_type = $this->quotes_model->getSaleByQuoteID($quote_ID);
			$qid = $sale_q->quote_id;
			 if (($this->quotes_model->getQuotesData($quote_ID)->status) != 'completed' ) {
				$this->session->set_flashdata('error', lang('can_not_add_sale'));
				redirect($_SERVER['HTTP_REFERER']);
			} 
			
			if($sale_q_type->type == 'quote'){
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    $product_tax += $pr_item_tax;
                    $subtotal = (($unit_price + $pr_item_tax) * $item_unit_quantity) - $pr_item_discount;
            ...
            if ($this->input->post('order_discount')) {
                ...
                if ($opos !== false) {
                    $ods = explode("%", $order_discount_id);
                    $order_discount = $this->erp->formatDecimal(((($total) * (Float) ($ods[0])) / 100), 4);
					//$order_discount = $order_discount * $item_quantity;
				} else {
                    $order_discount = $this->erp->formatDecimal($order_discount_id);
                }
            } else {
        ...
        if ($this->form_validation->run() == true && $this->sales_model->updateSale($id, $data, $products)) {
			...
			if ($this->data['inv']->sale_status == 'returned') {
                $this->session->set_flashdata('error', lang("can_not_edit_status_returned"));
                redirect($_SERVER["HTTP_REFERER"]);
            }
    ...
    function return_sale($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    $product_tax += $pr_item_tax;
                    $subtotal = (($unit_price * $item_quantity) + $pr_item_tax) -  $pr_discount;
        ...
        if ($this->form_validation->run() == true && $this->sales_model->returnSale($data, $products, $payment)) {
            ...
            if ($this->data['inv']->sale_status != 'completed') {
                $this->session->set_flashdata('error', lang("invoice_has_been_returned"));
                redirect($_SERVER["HTTP_REFERER"]);
            }
			...
			$sale_item = $this->sales_model->getInvoiceByID($id);
            $this->data['inv_items'] = json_encode($pr);
            $this->data['id'] = $id;
            $this->data['payment_ref'] = $this->site->getReference('sp');
			$this->data['billers'] = $this->site->getAllCompanies('biller');
            $this->data['warehouses'] = $this->site->getWarehouseByID($sale_item->warehouse_id);
            $this->data['tax_rates'] = $this->site->getAllTaxRates();
			$this->data['agencies'] = $this->site->getAllUsersById($sale_item->created_by);
			$this->data['customers'] = $this->site->getCustomers();
			$this->data['currency'] = $this->site->getCurrency();
    ...
    function suggestions()
    {
        ...
        if ($rows) {
            foreach ($rows as $row) {
                ...
                $row->real_unit_price = $this->erp->formatDecimal($row->price);
