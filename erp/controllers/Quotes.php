...
class Quotes extends MY_Controller
{
    ...
    public function getQuotes($warehouse_id = null)
    {
        ...
        $convert_link = anchor('sales/add/0/0/$1', '<i class="fa fa-heart"></i> ' . lang('create_sale'));
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("quotes.id, quotes.date, quotes.reference_no, users.first_name, quotes.biller, quotes.customer, quotes.grand_total, quotes.status")
                ->from('quotes')
				->join('users','users.id = quotes.created_by', 'left')
		...
		if($this->GP['quotes-allow_record']){
