...
class Purchases extends MY_Controller
{
    ...
    public function getPurchases($warehouse_id = null)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("purchases.id,purchases.date,purchases.reference_no, purchases.supplier,purchases.status, purchases.grand_total,purchases.paid, (erp_purchases.grand_total - erp_purchases.paid) as balance,purchases.payment_status")
                ->from('purchases')
...
//generate pdf and force to download
        ...
        if ($this->form_validation->run() == true) {
			...
			if($setting->accounting_method == 2 || $shipping){
				...
				for($i = 0; $i < $c; $i++){
					...
					$costunit = $this->site->calculateAverageCost($products[$i]['product_id'], $unitCost, $products[$i]['quantity'], $c, $products[$i]['item_discount'], $discounts, $shipping, $t_po_item_amount, $item_option);
	...
	public function edit_purchase_order($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->updatePurchaseOrder($id, $data, $products, $payment)) {
            $this->session->set_userdata('remove_pols', 1);
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->updatePurchase($id, $data, $products, $payment)) {
			/*
                ...
                foreach ($final as $csv_pr) {
                    if (!empty($csv_pr['code']) && !empty($csv_pr['quantity'])) {
						...
						if ($product_details = $this->purchases_model->getProductByCode(trim($csv_pr['code']))){
							...
							if(!$this->site->checkReference($csv_pr['reference_no'])){
							...
							{	
								$this->session->set_flashdata('error', $this->lang->line("same_ref") . " ( " . $csv_pr['reference_no'] . " )");
								redirect($_SERVER["HTTP_REFERER"]); 
    ...
    public function getPurchaseOrder($warehouse_id = null)
    {
        ...
        $detail_link = anchor('purchases/view_po/$1', '<i class="fa fa-file-text-o"></i> ' . lang('purchase_details'));
        $payments_link = anchor('purchases/payments/$1', '<i class="fa fa-money"></i> ' . lang('view_payments'), 'data-toggle="modal" data-target="#myModal"');
        ...
        $add_link = anchor('purchases/order_2_po/$1', '<i class="fa fa-edit add_or"></i> ' . lang('add_purchase'));
		$pdf_link = anchor('purchases/pdf/$1', '<i class="fa fa-file-pdf-o"></i> ' . lang('download_pdf'));
        $print_barcode = anchor('products/print_barcodes/?purchase=$1', '<i class="fa fa-print"></i> ' . lang('print_barcodes'));
    ...
    public function add_purchase_order($quote_id = null)
    {
        $this->erp->checkPermissions();
        $this->form_validation->set_message('is_natural_no_zero', $this->lang->line("no_zero_required"));
        //$this->form_validation->set_rules('reference_no', $this->lang->line("ref_no"), 'required');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $unit_cost = "unit_cost";
            $tax_rate = "tax_rate";
            $reference = $this->input->post('reference_no') ? $this->input->post('reference_no') : $this->site->getReference('poa');
			$payment_term = $this->input->post('payment_term');
            if ($this->Owner || $this->Admin) {
                $date = $this->erp->fld(trim($this->input->post('date')));
            } else {
        ...
        if ($this->form_validation->run() == true) {
			$purchase_order_id = $this->purchases_model->addPurchaseOrder($data, $products, $payment);
			if($this->Settings->purchase_serial){
				$this->purchases_model->addSerial($serial);
			}
			$this->session->set_userdata('remove_pols', 1);
            ...
            $this->data['tax_rates'] = $this->site->getAllTaxRates();
            $this->data['warehouses'] = $this->site->getAllWarehouses();
            $this->data['ponumber'] = $this->site->getReference('poa');
            $this->load->helper('string');
            $value = random_string('alnum', 20);
			$this->data['payment_ref'] = $this->site->getReference('pp');
            $this->session->set_userdata('user_csrf', $value);
            $this->data['csrf'] = $this->session->userdata('user_csrf');
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('purchases'), 'page' => lang('purchases')), array('link' => '#', 'page' => lang('add_purchase_order')));
