...
class Products extends MY_Controller
{
    ...
    function getProducts($warehouse_id = NULL)
    {
        ...
        $action .= '<li><a href="' . site_url() . 'assets/uploads/$2" data-type="image" data-toggle="lightbox"><i class="fa fa-file-photo-o"></i> '
            ...
            . lang('adjust_quantity') . '</a></li>
				<li class="divider"></li>
				<li>' . (($this->Owner || $this->Admin) ? '<li>'.$edit_link.'</li>' : ($this->GP['products-edit'] ? '<li>'.$edit_link.'</li>' : '')).(($this->Owner || $this->Admin) ? '<li class="divider"></li><li>'.$delete_link.'</li>' : ($this->GP['products-delete'] ? '<li>'.$delete_link.'</li>' : '')). '</li>
			</ul>
		</div></div>';
    ...
    {
        $this->erp->checkPermissions('print_barcode', null, 'products');
	...
	function product_actions($wh = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
                ...
                }else if ($this->input->post('form_action') == 're_active') {
                    foreach ($_POST['val'] as $id) {
                        $this->products_model->updateActiveProducts($id);
                    }
                    $this->session->set_flashdata('message', $this->lang->line("product_successfully_reactive"));
                    redirect($_SERVER["HTTP_REFERER"]);
                    
                }else if ($this->input->post('form_action') == 'in_active') {
                    foreach ($_POST['val'] as $id) {
                        $this->products_model->updateInactiveProducts($id);
                    }
                    $this->session->set_flashdata('message', $this->lang->line("product_successfully_inactive"));
                    redirect($_SERVER["HTTP_REFERER"]);
		...
		if ($this->form_validation->run() == true) {
			if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    $this->load->library('excel');
                    $this->excel->setActiveSheetIndex(0);
                    $this->excel->getActiveSheet()->setTitle(lang('count_stock'));
                    $this->excel->getActiveSheet()->SetCellValue('A1', lang('product_code'));
                    $this->excel->getActiveSheet()->SetCellValue('B1', lang('product_name'));
                    $this->excel->getActiveSheet()->SetCellValue('C1', lang('category_name'));
					$this->excel->getActiveSheet()->SetCellValue('D1', lang('expected'));
					$this->excel->getActiveSheet()->SetCellValue('E1', lang('counted'));
					$this->excel->getActiveSheet()->SetCellValue('F1', lang('difference'));
                    $row = 2;
					$get_arr_id = $_POST['val'];
					foreach($get_arr_id as $arr_id){
						$id = $arr_id;
					}					
					$stock_count = $this->products_model->getStouckCountByID($id);					
					$stock_count_items = $this->products_model->getStockCountItems($id);
					$warehouse_product = $this->products_model->getProductByWareIdAndCateId($stock_count->warehouse_id, $stock_count->category_id);
					$products = array();
					...
					foreach($warehouse_product as $sc){
						$counted = 0;						
						if($products[$sc->code]['counted']){
							$counted = $products[$sc->code]['counted'];
						}
						$category_name = $this->products_model->getCategoryById($sc->category_id)->name;
						$this->excel->getActiveSheet()->SetCellValue('A' . $row, $sc->code);
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $sc->label);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $category_name);
						$this->excel->getActiveSheet()->SetCellValue('D' . $row, $sc->quantity);
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, $counted);
						$this->excel->getActiveSheet()->SetCellValue('F' . $row, $counted - $sc->quantity);
                        $row++;
                    }
	...
	function getCounts($warehouse_id = NULL)
    {
        ...
        $this->load->library('datatables');
        $this->datatables
            ->select("{$this->db->dbprefix('stock_counts')}.id as id, date, reference_no, {$this->db->dbprefix('warehouses')}.name as wh_name, users.username, note, final_file")
            ->from('stock_counts')
            ->join('users', 'users.id = stock_counts.created_by', 'left')
            ->join('warehouses', 'warehouses.id=stock_counts.warehouse_id', 'left');
        if ($warehouse_id) {
            $this->datatables->where('stock_counts.warehouse_id', $warehouse_id);
        }
	...
	function view_count($id)
    {
        $this->erp->checkPermissions('count_stock',null,'products');
        $stock_count = $this->products_model->getStouckCountByID($id);
		$categories = $this->products_model->getCategoryArrayId($stock_count->category_id);
		$get_category = array();
		foreach($categories as $cate){
			$get_category[] = $cate->name;
		}
		$category_name = implode('  |  ', $get_category);
        ...
        $this->data['warehouse_product'] = $this->products_model->getProductByWareIdAndCateId($stock_count->warehouse_id, $stock_count->category_id);
    ...
    function finalize_count()
    {
		...
		if ($this->form_validation->run() == true) {
            ...
            if ($_FILES['csv_file']['size'] > 0) {
				...
				$reference = $this->input->post('reference_no') ? $this->input->post('reference_no') : $this->site->getReference('st');
				$categories_id = '';
				$i = 1;
                ...
                $keys = array('product_code', 'counted');
                $final = array();				
                foreach ($arrResult as $key => $value) {
                    $final[] = array_combine($keys, $value);
                } 
				
                $rw = 0; $differences = 0; $matches = 0; $missing = 0;
                foreach ($final as $pr) {
                    if ($product = $this->products_model->getProductByCodeAndCateId(trim($pr['product_code']), $categories_id)) {
						$product_qty = $this->products_model->getProductQuantityById($product->id, $warehouse_id);
						$warehouse = $this->products_model->getWarehouseProduct($product->id);
							$pr['product_id'] = $product->id;
							$pr['product_code'] = $product->code;
							...
							$pr['expected'] = $product_qty->quantity ? $product_qty->quantity : 0;
                            $pr['cost'] = $product->cost;
							
							if($product_qty->quantity == $pr['counted']){
								$matches++;
							}
							...
							if($pr['counted'] > $product_qty->quantity){
								 $missing++;
							}
                        ...
                        redirect('products/finalize_count/');
                    }
                    $rw++;
                }				
                $data['final_file'] = $csv;
        ...
        if ($this->form_validation->run() == true && $this->products_model->finalizeStockCount($data, $products)) {
            ...
            $this->data['stoct_count_ref'] = $this->site->getReference('st');
			$this->data['categories']   = $this->site->getAllCategories();
