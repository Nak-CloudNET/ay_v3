...
class Customers extends MY_Controller
{
    ...
    function getCustomers()
    {
		...
		$this->datatables
            ->select("id, id AS cus_no, company, name, email, phone, customer_group_name, vat_no, deposit_amount, award_points, attachment")
            ->from("companies")
	...
	function check_phone_available($term = NULL){
		$term = $this->input->get('term', TRUE);
        if (strlen($term) < 1 || !$term) {
            die("<script type='text/javascript'>setTimeout(function(){ window.top.location.href = '" . site_url('welcome') . "'; }, 10);</script>");
        }
		$row = $this->companies_model->getPhoneCustomer($term);
        if ($row) {
            echo 1;
        } else {
            echo 0;
	...
	function suggestionsCustomerGroup($term)
    {
        if ($this->input->get('term')) {
            $term = $this->input->get('term', TRUE);
        }
		...
		$customer = $this->site->getCompanyByID($term);
        $customer_group = $this->site->getCustomerGroupByID($customer->customer_group_id);
        $rows['results'] = $customer_group->percent;
        echo json_encode($rows);
    }
    ...
    function getCustomer($id = NULL)
    {
        ...
        echo json_encode(array(array('id' => $row->id, 'text' => ($row->name != '-' ? $row->name .'('. $row->customer_group_name .')' : $row->company))));
    }
