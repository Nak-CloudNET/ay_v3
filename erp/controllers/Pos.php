...
class Pos extends MY_Controller
{
    ...
    function index($sid = null)
    {
        ...
        if ($this->form_validation->run() == true) {
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    if (isset($item_discount)) {
                        ...
                        if ($dpos !== false) {
							...
							$pr_discount = $pr_discount * $item_quantity;
							$discount_pr = $this->erp->formatMoney((($unit_price * $item_quantity) * (Float)($pds[0])) / 100);
                        } else {
                            $pr_discount = $this->erp->formatDecimal($discount);
							$discount_pr = $this->erp->formatDecimal($discount);
                        }
                    }
                    ...
                    $subtotal = (((($unit_price + $pr_discount) * $item_quantity) - $discount_pr) + $pr_item_tax);
                    ...
                    $products[] = array(
                        ...
                        'unit_price' => $this->erp->formatMoney($item_net_price + $pr_discount + $item_tax),
                        ...
                        'subtotal' => $this->erp->formatMoney($subtotal),
                        'serial_no' => $item_serial,
                        'real_unit_price' => $real_unit_price,
						'product_noted' => $item_note
                    );
                    $total += (($unit_price + $pr_discount) * $this->erp->formatDecimal($item_quantity) - $discount_pr);
            ...
            if ($this->input->post('discount') || $this->input->post('percent_cus_group')) {
                $order_discount_id = $this->input->post('discount') ? $this->input->post('discount') : $this->input->post('percent_cus_group') ;
            ...
            $total_tax = $this->erp->formatMoney($product_tax + $order_tax);
            $grand_total = $this->erp->formatMoney($total + $total_tax + $shipping - $order_discount);
        ...
        if ($this->form_validation->run() == true && !empty($products) && !empty($data)) {
			...
			$this->data['agencies'] = $this->site->getAllUsers();
			$data_users = $this->site->getAllUsersAndGroup();
			$get_user_id = array();
			foreach($data_users as $user){
				if($user->group_name == 'admin' || $user->group_name == 'owner' || $user->group_name == 'accountant'){
					$get_user_id[] = $user->id;					
				}
			}
			$user_id = implode('_', $get_user_id);
    ...
    function close_register($user_id = NULL, $user_close_by = NULL)
    {
        $this->erp->checkPermissions('index');
        if (!$this->Owner && !$this->Admin) {
            $user_id = $this->session->userdata('user_id');
		...
		if ($this->form_validation->run() == true) {
            ...
            $data = array('closed_at' => date('Y-m-d H:i:s'),
                ...
                'total_cash_submitted' => $this->input->post('hidden_total_cash'),
                ...
                'closed_by' => $this->input->post('user_close_by')
            );			
        ...
        if ($this->form_validation->run() == true && $this->pos_model->closeRegister($rid, $user_id, $data)) {
            ...
            $this->data['user_id'] = $user_id;
            $this->data['user_close_by'] = $user_close_by;
    ...
    function add_payment($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $payment = array(
				...
				'biller_id' 	=> $this->pos_settings->default_biller,
