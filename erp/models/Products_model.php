...
class Products_model extends CI_Model
{
	...
	public function updateActiveProducts($id)
    {        
        if ($this->db->update('products', array('inactived' => 0), array('id' => $id))) {
            return TRUE;
        }
        return FALSE;
    }
	
	public function updateInactiveProducts($id)
    {        
        if ($this->db->update('products', array('inactived' => 1), array('id' => $id))) {
            return TRUE;
        }
        return FALSE;
    }
	...
	public function getCategoryArrayId($id)
    {
	...
	public function getProductByCodeAndCateId($code, $cate_id = NULL)
    {
		if($cate_id){
			$category_id = explode(',', $cate_id);
	...
	public function getProductQuantityById($product_id, $warehouse)
    {
    ...
    */
    ...
    public function finalizeStockCount($data, $products)
    {
       if ($this->db->insert('stock_counts', $data)) {
			$stock_counts_id = $this->db->insert_id();
			if ($this->site->getReference('st') == $data['reference_no']) {
				$this->site->updateReference('st');
			}
			foreach ($products as $item) {
				$item['stock_count_id'] = $stock_counts_id;
				$this->db->insert('stock_count_items', $item);
			}
			return true;
	   }
        return FALSE;
	...
	public function getProductDataByWareId($w_id=null, $product_id = null)
    {	
		if($product_id){
    ...
    {	
		...
		$this->db->where(array('warehouses_products.warehouse_id'=>$w_id, 'products.inactived !='=>'1'));
		$this->db->select('products.id as pid, products.code, products.cost, products.name as label, COALESCE(erp_product_variants.name, "") as variant, warehouses_products.quantity as quantity, 0 as qty');
        $this->db->from('products');
		$this->db->join('warehouses_products' ,'warehouses_products.product_id=products.id', 'left');
		$this->db->join('product_variants' ,'product_variants.product_id=products.id', 'left');
		$q = $this->db->get();
	...
	public function getProductByWareIdAndCateId($w_id=null, $c_id = null)
    {	
		if($c_id){
			$cate_id = explode(',', $c_id);
			$this->db->where_in('products.category_id',$cate_id);
		}
		$this->db->where(array('warehouses_products.warehouse_id'=>$w_id, 'products.inactived !='=>'1'));
		$this->db->select('products.id as pid, products.code, products.category_id, products.cost, products.name as label, COALESCE(erp_product_variants.name, "") as variant, warehouses_products.quantity as quantity, 0 as qty');
        $this->db->from('products');
		$this->db->join('warehouses_products' ,'warehouses_products.product_id=products.id', 'left');
		$this->db->join('product_variants' ,'product_variants.product_id=products.id', 'left');
		$q = $this->db->get();
