...
SELECT
	...
	public function getCosting($date)
    {
        $this->db->select('SUM( COALESCE( erp_products.cost * (erp_sale_items.quantity - COALESCE(erp_return_items.quantity, 0)), 0 ) ) AS cost, 
                            SUM( COALESCE( erp_sale_items.quantity - COALESCE(erp_return_items.quantity, 0), 0 ) ) AS total_items ', FALSE)
	...
	public function getExpense($date, $warehouse_id = NULL, $year = NULL, $month = NULL)
    {
        $sdate = $date.' 00:00:00';
        $edate = $date.' 23:59:59';
        $this->db->select('COUNT( COALESCE( id, 0 ) ) AS count_ex, SUM( COALESCE( amount, 0 ) ) AS total', FALSE);
        if ($date) {
            $this->db->where('date >=', $sdate)->where('date <=', $edate);
        } elseif ($month) {
            $this->load->helper('date');
...
*/
	...
	public function getShopsByID($id){
		$this->db->select("erp_companies.id as idd,company,.erp_companies.name,erp_companies.phone,erp_companies.name,count(erp_sales.id) as total,COALESCE(SUM(erp_sales.grand_total),0) as grand_total,(COALESCE(SUM(erp_sales.grand_total),0)*(erp_companies.cf6/100))as total_earned,COALESCE(sum(paid), 0) as paid, ( COALESCE(sum(grand_total), 0) - COALESCE(sum(paid), 0)) as balance")
		->join('sales', 'sales.biller_id=companies.id')
        ->where('companies.group_name', 'biller');
		$q =$this->db->get_where('erp_companies',array('companies.group_name'=>'biller'));
		if($q->num_rows() > 0){
			return $q->row();
		}
		return false;
	...
	function getDataReportDetail($id, $start= NULL, $end = NULL, $biller = NULL){
		if($id){
			...
			if($biller){
					$biller_id = "AND erp_sales.warehouse_id IN($biller)";
			}
		$q = $this->db->query("
							SELECT
								`erp_sales`.`id`,
								...
								SUM(COALESCE(
										((COALESCE(SUBSTRING_INDEX(`erp_sales`.`order_discount_id`, '%', 1),0) * (COALESCE((`erp_sale_items`.`subtotal`),0)))/100) 
								,0)) AS `order_Dis`,
								...
								COALESCE(COALESCE(`erp_return_items`.`subtotal`,0) - (COALESCE(SUBSTRING_INDEX(`erp_return_sales`.`order_discount_id`, '%', 1),0) * (COALESCE(`erp_return_items`.`real_unit_price`, 0) * COALESCE(`erp_return_items`.`quantity`, 0)))/100 ,0) AS `refund`
							FROM
								`erp_sales`
        ...
        foreach($q as $data_row){
			...
			$profit = $data_row->revenue - ($this->erp->formatDecimal($data_row->order_Dis) + $data_row->coms + $data_row->refund);
			$sum_profit+=$profit;
			$sum_order_dis+=$data_row->order_Dis;
			$sum_refund+=$data_row->refund;
			$row.='<tr style="text-align:center !important;">';					
				$row.='<td>'.$data_row->productName.'</td>';
				$row.='<td style="text-align:center !important;">'.$this->erp->formatQuantity($data_row->stockInHand).'</td>';
				$row.='<td style="text-align:center !important;">'.$this->erp->formatQuantity($data_row->saleQuantity) .'</td>';
				$row.='<td style="text-align:center !important;">'.$this->erp->formatQuantity($data_row->saleReturnQuantity) .'</td>';
				$row.='<td style="text-align:center !important;">'.$this->erp->formatMoney($data_row->unitCost).'</td>';
				$row.='<td style="text-align:center !important;">'.$this->erp->formatMoney($data_row->unitPrice) .'</td>';
				$row.='<td style="text-align:center !important;">'.$this->erp->formatMoney($data_row->item_Dis) .'</td>';
				$row.='<td style="text-align:center !important;">'.$this->erp->formatMoney($data_row->order_Dis) .'</td>';
				$row.='<td style="text-align:center !important;">'.$this->erp->formatMoney($data_row->revenue) .'</td>';
				$row.='<td style="text-align:center !important;">'.$this->erp->formatMoney($data_row->coms) .'</td>';
				$row.='<td style="text-align:center !important;">'.$this->erp->formatMoney($data_row->refund) .'</td>';
				$row.='<td style="text-align:center !important;">'.$this->erp->formatMoney($profit) .'</td>';
			$row.='</tr>';			
		}
		$row.='<tr style="font-weight:bold;">';					
				$row.='<td style="background-color:pink;text-align:right;padding-right:10px;">'."Grand Total".'</td>';
				$row.='<td class="stockInHand" style="text-align:center !important;">'.$this->erp->formatQuantity($stockInHand) .'</td>';
				$row.='<td class="saleQuantity" style="text-align:center !important;">'.$this->erp->formatQuantity($saleQuantity) .'</td>';
				$row.='<td class="saleReturnQuantity" style="text-align:center !important;">'.$this->erp->formatQuantity($saleReturnQuantity) .'</td>';
				$row.='<td class="unitCost" style="text-align:center !important;">'.$this->erp->formatMoney($unitCost).'</td>';
				$row.='<td class="unitPrice" style="text-align:center !important;">'.$this->erp->formatMoney($unitPrice) .'</td>';
				$row.='<td class="item_dis" style="text-align:center !important;">'.$this->erp->formatMoney($item_Dis) .'</td>';
				$row.='<td class="order_dis" style="text-align:center !important;">'.$this->erp->formatMoney($sum_order_dis) .'</td>';
				$row.='<td class="revenue" style="text-align:center !important;">'.$this->erp->formatMoney($revenue) .'</td>';
				$row.='<td class="coms" style="text-align:center !important;">'.$this->erp->formatMoney($coms) .'</td>';
				$row.='<td class="refund" style="text-align:center !important;">'.$this->erp->formatMoney($sum_refund) .'</td>';
				$row.='<td class="profit" style="text-align:center !important;">'.$this->erp->formatMoney($sum_profit) .'</td>';											
			$row.='</tr>';
		return $row;
	  }
	else		
	...
	function getCategoryName($category_id = NULL, $product_id = NULL, $start = NULL, $end = NULL, $ch = array(), $saleman = NULL, $biller = NULL){
		...
		if($biller){
			$billers = explode(',', $biller);
			$this->db->where_in('sales.warehouse_id', $billers);
		}
	...
	function getExportSaleDetail($id,$start,$end){
		$this->db->select("sales.id,categories.id as categoryId,categories.name as categoryName,CONCAT(erp_sale_items.product_name,' (',erp_sale_items.product_code,')') as productName,products.quantity as stockInHand,SUM(erp_sale_items.quantity) AS saleQuantity,SUM(COALESCE(erp_return_items.quantity,0)) AS saleReturnQuantity,SUM(erp_sale_items.item_discount) AS item_Dis,SUM(COALESCE(((COALESCE(SUBSTRING_INDEX(erp_sales.order_discount_id, '%', 1),0) * (COALESCE(erp_sale_items.unit_price,0) * COALESCE(erp_sale_items.quantity,0)))/100),0)) AS order_Dis,products.cost as unitCost,erp_sale_items.unit_price as unitPrice,SUM(erp_sale_items.subtotal) as revenue,COALESCE((SUM(erp_sale_items.quantity - COALESCE(erp_return_items.quantity,0)) * erp_products.cost), 0) AS coms, COALESCE(COALESCE(erp_return_items.subtotal,0) - (COALESCE(SUBSTRING_INDEX(erp_return_sales.order_discount_id, '%', 1),0) * (COALESCE(erp_return_items.real_unit_price, 0) * COALESCE(erp_return_items.quantity, 0)))/100 ,0) AS refund");
        $this->db->from('sales');
        $this->db->join('sale_items', 'sales.id = sale_items.sale_id');
		$this->db->join('return_items','return_items.sale_item_id=sale_items.id','left');
        ...
        $this->db->group_by('sales.order_discount_id');			
        $this->db->group_by('sale_items.discount');		
        $this->db->group_by('return_sales.order_discount_id');		
        $this->db->group_by('erp_return_items.discount');
        $this->db->group_by('erp_return_items.sale_id');
		$q = $this->db->get();
		if($q->num_rows() > 0 ) {
			return $q->result();
		}
