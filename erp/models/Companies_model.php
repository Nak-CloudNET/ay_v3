...
class Companies_model extends CI_Model
{
	...
	public function getPhoneCustomer($term)
    {
        $this->db->select('phone');
		$q = $this->db->get_where('companies', array('phone' => $term), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    }
    ...
    {
        $this->db->select("id, (CASE WHEN (LENGTH(CONCAT(name, ' (', customer_group_name, ')')) > 50) THEN CONCAT(LEFT (CONCAT(name, ' (', customer_group_name, ')'), 50), '...') ELSE CONCAT(name, ' (', customer_group_name, ')') END) as text", FALSE);
        $this->db->where(" (id LIKE '%" . $term . "%' OR name LIKE '%" . $term . "%' OR customer_group_name LIKE '%" . $term . "%' OR email LIKE '%" . $term . "%' OR phone LIKE '%" . $term . "%') ");
